...
class Arpegio(AbstractProtocol):
    ...
    async def find_peer(self, pid):
        return None
    async def add_a_new_peer(self, stage, pid, counter):
        if pid != self.peer.id_node:
            if self.peers.get(pid) != None:
                
                return self.update_peer(stage,pid,counter)
            ...
            self.peers[pid] = pobj
            self.calculate_cost()
    def update_peer(self, stage, pid, counter):
        # return
        if pid == self.peer.id_node:
            return
        if self.peers.get(pid) != None:
    ...
    def process_datagram(self, addr: tuple[str, int], data: bytes):
        if data[0] == Arpegio.INTRODUCTION:
            ...
            pid = data[1:33]
            loop = asyncio.get_event_loop()
            loop.create_task(self.add_a_new_peer(stage, pid, counter))
...
# wrong stage
                self.counter += 1
                self.accept_proposal(addr, my_same_max, their_stage)
                self.T = 0.95*self.T
    ...
    async def periodic(self):
        ...
        for pt in diff_stages:
            ...
            for p in prvpprs + nxtprs:
                ...
                tmp_proof.append((p,100*self.costmap(self.peer.pub_key, self.get_peer(p.p.id_node).pub_key), p.counter))
